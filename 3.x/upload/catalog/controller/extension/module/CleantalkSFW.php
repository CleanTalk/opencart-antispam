...
class CleantalkSFW extends CleantalkHelper
{
	...
	{
		$this->db_result_data = $this->db_result->rows[0];
	}
	
	public function unversal_fetch_all()
	{
		$this->db_result_data = $this->db_result->rows;
	}
	...
	*/	
	public function sfw_die($api_key, $cookie_prefix = '', $cookie_domain = ''){
		...
		// File exists?
		if(file_exists(DIR_APPLICATION . '../catalog/controller/extension/module/sfw_die_page.html')){
			$sfw_die_page = file_get_contents(DIR_APPLICATION . '../catalog/controller/extension/module/sfw_die_page.html');
		}else{
			die("IP BLACKLISTED");
		}
